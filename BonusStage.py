...
def bonus_stage(skip_bonus_stage_state):
    ...
    if skip_bonus_stage_state:
        ...
        if not pyautogui.pixelMatchesColor(
            454, 91, (225, 224, 226)
        ):  # if Spirit Boost, do nothing until close appears
            bonus_stage_nsp()
        else:
            bonus_stage_nsp()
...
def bonus_stage_fail():
    ...
    if pyautogui.pixelMatchesColor(
        window.left + 775, window.top + 600, (180, 0, 0), tolerance=10
    ):
...
def c_send(press_delay, post_press_delay=0, key=Key.up):
    keyboard.press(key)
...
def slider():
    ...
    for pos in positions:
        if pyautogui.pixelMatchesColor(
            window.left + pos["x"], window.top + pos["y"], (0, 126, 0)
        ):
...
@timer
def find_pixel_until_found(x1, y1, x2, y2, color, timer=15):
