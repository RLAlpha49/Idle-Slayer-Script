...
default_settings = {
    ...
    "autobuyvalue": "600",
    "cycleportalcount": "1",
    "autoascensionslider": "20",
    "ragestate": "1",
    "autobuyupgradestate": "False",
    "cycleportalsstate": "False",
    "skipbonusstagestate": "True",
    "craftsoulbonusstate": "False",
    "craftragepillstate": "False",
    "craftdimensionalstaffstate": "False",
    "craftbidimensionalstaffstate": "False",
    "quicktoolbeststate": "False",
    "disableragehordestate": "False",
    "nolockpicking100state": "True",
    "chesthuntactivestate": "False",
    "autoascensionstate": "False",
    "ragesoulbonusstate": "False",
    "slayerpoints": "False",
}
...
default_stats = {
    "Rage with only MegaHorde": "0",
    "Rage with Soul Bonus": "0",
    "Rage with MegaHorde and Soul Bonus": "0",
    "Claimed Quests": "0",
    "Claimed Minions": "0",
    "ChestHunts": "0",
    "Perfect ChestHunts": "0",
    "Collected Silver Boxes": "0",
    "Portals Cycled": "0",
    "Failed/Skipped Bonus Stages": "0",
}
...
class MyTabView(customtkinter.CTkTabview):
    ...
    def create_tab_content(self, tab_name):
        ...
        general_description_label = customtkinter.CTkLabel(
            master=tab,
            text="Jump Rate(ms)",
        ...
        update_button = customtkinter.CTkButton(
            master=tab, text="Update Settings", command=self.update_settings
        )
        ...
        checkbox_tooltips = [
            [
                "Will automatically buy equipment/upgrades\nWill activate 10 seconds after turning on",
                "Cycle's through portal whenever available",
            ],
            [
                "Will go through bonus stage letting time run out",
                "Check if you have Lockpicking100, for faster chesthunts",
            ],
            [
                "Tooltip for Craft dimensional Staff (WIP)",
                "Tooltip for Craft bidimensional Staff (WIP)",
            ],
        ]
        ...
        for col, label_text in enumerate(label_texts, start=0):
            ...
            for row, checkbox_text in enumerate(checkbox_texts[col], start=1):
                ...
                checkbox_state = self.settings.getboolean(
                    "Settings", checkbox_state_key, fallback=False
                )
                if checkbox_state is True:
                    checkbox.select()

                checkbox_tooltip = checkbox_tooltips[col][
                    row - 1
                ]  # Get the corresponding tooltip
                CTkToolTip(
                    checkbox, message=checkbox_tooltip
                )  # Create a CTkToolTip instance for each checkbox

                checkbox_column.append(checkbox)

            self.checkboxes.append(
                checkbox_column
            )  # Add the checkbox column to the checkboxes list
    ...
    def create_tab_2_content(self, tab_name):
        ...
        self.label1 = customtkinter.CTkLabel(
            master=tab, text="Rage", font=customtkinter.CTkFont(size=16, weight="bold")
        )
        ...
        combobox_options = [
            "Rage at Megahorde with Soul Bonus",
            "Rage at Megahorde",
            "Rage with Soul Bonus",
        ]
        self.combobox = customtkinter.CTkComboBox(
            master=tab,
            values=combobox_options,
            state="readonly",
            command=self.update_combobox_value,
        )
        ...
        self.checkbox1_var.set(
            settings.getboolean("Settings", "CraftSoulBonusState", fallback=False)
        )
        CTkToolTip(
            checkbox1,
            message="Will craft Soul Bonus when rage is activated\nWill do this regardless of rage setting",
        )

        self.checkbox2_var = customtkinter.IntVar()
        checkbox2 = customtkinter.CTkCheckBox(
            master=tab,
            text="Craft Rage Pill",
        ...
        self.checkbox2_var.set(
            settings.getboolean("Settings", "CraftRagePillState", fallback=False)
        )
        CTkToolTip(
            checkbox2,
            message="Will craft Rage Pill if rage has cooldown\nWill do this regardless of rage setting",
        )
        ...
        self.general_text_box2 = customtkinter.CTkTextbox(
            master=tab,
            height=5,
            width=75,
            font=customtkinter.CTkFont(size=13),
        ...
        CTkToolTip(
            self.general_text_box2,
            message="How often the Auto Buy Function should be run in Seconds",
        )

        self.update_button = customtkinter.CTkButton(
            master=tab,
            text="Update Auto Buy Timer",
            height=25,
            command=self.update_auto_buy_value,
            font=customtkinter.CTkFont(size=13),
        )
        self.update_button.grid(row=2, column=3, padx=20, pady=(0, 0))

        self.belt_checkbox_var = customtkinter.IntVar()
        self.belt_checkbox = customtkinter.CTkCheckBox(
            master=tab,
            text="Quick Tool Belt",
            variable=self.belt_checkbox_var,
            command=self.update_settings,
            font=customtkinter.CTkFont(size=13),
            checkbox_height=20,
            checkbox_width=20,
        ...
        self.belt_checkbox_var.set(
            settings.getboolean("Settings", "quicktoolbeltstate", fallback=False)
        )
        CTkToolTip(
            self.belt_checkbox,
            message="Will craft items using the quick tool belt\nWill use original crafting tab if the items are not found in belt",
        )
    ...
    def update_combobox_value(self, *args):
        ...
        selected_ragestate = ragestate_mapping.get(
            selected_option, 1
        )  # Default to 1 if option not found
    ...
    def update_auto_buy_value(self):
        ...
        current_auto_buy_value = self.settings.get(
            "Settings", "AutoBuyValue", fallback=""
        )
    ...
    def update_settings(self):
        ...
        checkbox_names = [
            ...
            "No Lockpicking100",
            "Rage Soul Bonus",
            "Craft dimensional Staff",
            "Craft bidimensional staff",
        ]
        for col, checkbox_column in enumerate(self.checkboxes):
            for row, checkbox in enumerate(checkbox_column, start=1):
                ...
                current_checkbox_state = settings.get(
                    "Settings", checkbox_state_key, fallback="False"
                )

                if new_checkbox_state != (current_checkbox_state == "True"):
                    settings.set(
                        "Settings", checkbox_state_key, str(new_checkbox_state)
                    )
                    write_log_entry(
                        f"Checkbox '{checkbox_names[col * 3 + row - 1]}' updated to: {new_checkbox_state}"
                    )
        ...
        settings.set(
            "Settings",
            "CraftSoulBonusState",
            "True" if (str(self.checkbox1_var.get()) == "1") else "False",
        )

        settings.set(
            "Settings",
            "CraftRagePillState",
            "True" if (str(self.checkbox2_var.get()) == "1") else "False",
        )

        settings.set(
            "Settings",
            "autoascensionstate",
            "True" if (str(self.checkbox3_var.get()) == "1") else "False",
        )

        settings.set(
            "Settings",
            "quicktoolbeltstate",
            "True" if (str(self.belt_checkbox_var.get()) == "1") else "False",
        )
...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        for row, text in enumerate(button_texts, start=0):
            ...
            button = customtkinter.CTkButton(
                self.sidebar_frame,
                text=text,
                command=lambda t=text: self.show_text(t),
    ...
    def create_general_frame_with_tabs(self):
        ...
        self.general_frame.grid(
            row=1, column=1, columnspan=3, padx=(0, 0), pady=(0, 0), sticky="nsew"
        )
    ...
    def create_home_frame(self):
        ...
        image1_tk = customtkinter.CTkImage(
            light_image=Image.open(image1dir), size=(240, 81.6)
        )
        image2_tk = customtkinter.CTkImage(
            light_image=Image.open(image2dir), size=(240, 83.48)
        )
        ...
        image_label1 = customtkinter.CTkLabel(
            self.home_frame, image=image1_tk, cursor="hand2", text=""
        )
        image_label1.bind("<Button-1>", lambda e: self.open_link1())
        image_label1.grid(row=2, column=0, padx=20, pady=0)

        image_label2 = customtkinter.CTkLabel(
            self.home_frame, image=image2_tk, cursor="hand2", text=""
        )
        image_label2.bind("<Button-1>", lambda e: self.open_link2())
        image_label2.grid(row=2, column=1, padx=20, pady=0)

    def open_link1(self):
        webbrowser.open("https://discordid.netlify.app/?id=251479989378220044")
    ...
    def create_log_frame(self):
        ...
        self.stats_text_box = customtkinter.CTkTextbox(
            self.log_frame, height=115, width=300
        )
        ...
        self.log_text_box = customtkinter.CTkTextbox(
            self.log_frame, height=115, width=300
        )
